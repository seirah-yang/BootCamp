...
# ===== 인덱서/검색기 ===== 텍스트 청크 임베딩 & FAISS를 이용해 벡터 검색 수행
class TextIndexer:
    ...
    def search_dense(self, query: str, topk=4):
        ...
        for d, i in zip(D[0], I[0]):
            if i < 0:
                continue
            it = dict(self.chunks[i])  # copy
            it["score"] = float(d)     # 코사인 유사도(내적)
            it["source"] = "dense"
            items.append(it)
        return items
...
class HybridRetriever:
    ...
    def search(self, query, k_dense=50, k_bm25=50, topk=10, rrf_k=60):
        ...
        if self.bm25 is not None and self.corpus:
            qtok = self.tok(query)
            scores = self.bm25.get_scores(qtok)  # 전체 문서 점수
...
try:
    ...
    exit()
# ===============================
# 2. 사용자 입력 받기(R&D 과제명에서 핵심 키워드 추출)
...
def generate_text(section, keywords=""):  # RAG 기반 텍스트 생성
    ...
    output = generator(
        prompt,
        max_new_tokens=5000,  # 220 -> 500->5000으로 변경
        do_sample=False,     # 사실성 우선
        temperature=0,
        top_p=0.9,
